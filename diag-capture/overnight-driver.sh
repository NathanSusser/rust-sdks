...
#
# Usage: overnight-driver.sh <night-label> <end_unix_s> [cell_s=600] [cap_kbps=2000]
set -uo pipefail
...
fails=0; k=0
log "start night=$night end=$(date -u -d @"$END_EPOCH" +%FT%TZ) cell=${CELL}s cap=${CAP}k h265_every=$H265_EVERY diag_every=$DIAG_EVERY build=$(readlink "$DIAG_DIR/qcsuper-noroot")"
while :; do
  now=$(date -u +%s)
  ...
  diag=0; [ $(( k % DIAG_EVERY )) -eq 1 ] && [ "$codec" = h264 ] && diag=1
